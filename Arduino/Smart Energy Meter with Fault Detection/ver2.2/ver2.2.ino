...
void setup()
{
  ...
  emon1.current(1, 1.8); //Initiallize Current sensor_1 for household meter
  emon2.current(0, 1.8); //Initiallize Current sensor_2 for Main PMT meter
float f1 = 12.5f;
float f2 = 12.5f;
EEPROM.put(11, f1);
EEPROM.put(22, f2);
...
//   // establishConnection("AT+CIOBAUD=9600",1500); //establishConnection("AT+UART_DEF=9600,0,0,0",1500);
// //establishConnection("AT+UART_DEF=9600,8,1,0,0",1500);
//    a++;
//  }
//  Serial2.end();
...
//    delay(250);
  ...
  if (content.substring(1) == "26 AC E9 25") //change UID of the card that you want to give access
  {
    Serial.println("Card");
    delay(1000);
    //user_flag_1 = 
  ...
  else if (content.substring(1) == "50 3B B2 2B" and user_flag_1 == 1)
  {
...
//    Serial.println(diff_current,4);
//    if (fabs(amps_2 - amps) > 0.007)
  
  //comapareValueFloat = data *10;
  //compareValue = round(comapareValueFloat);
//  compareValue = round(data);
//  //Serial.println(compareValue);
//  if (compareValue < oldCompareValue)
//  {
//    Serial.println("Your units less than 11");
//  }
...
{
    if (temp == "26 AC E9 25") //change UID of the card that you want to give access
  {
...
//  for(int i=0;i<2;i++)
//  {
//    sample1+=analogRead(A10);  //read the voltage from the sensor
//    //sample2+=analogRead(A1); //read the current from sensor
//    sample2+= emon2.calcIrms(1480);
//    delay(2);
//  }
//   sample1=sample1/2; 
//   sample2=sample2/2;
...
//   avgamps=totamps/temp; // average amps
//   amphr=(avgamps*temp)/3600;  // amphour
//   watt =voltage*amps;    // power=voltage*current
//   //energy=(watt*time)/3600;   //energy in watt hour
//   energy=(watt*temp)/(1000*3600);   // energy in kWh
//   energy = abs(energy);
...
//   amps =actualval*10; // 100mv/A from data sheet 
    ...
    float diff_current = fabs(amps_2 - amps);
    Serial.println(diff_current,4);
    if (fabs(amps_2 - amps) > 0.005)  // 0.007 and 0.006 both are working in most cases 
    {
